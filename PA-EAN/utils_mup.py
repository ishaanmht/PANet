...
def embed_pref(pref, from_, to_, is_training, num_units, BN=True, initializer=tf.contrib.layers.xavier_initializer()):
    with tf.variable_scope("embedding_pref"): # embed + BN input set
        
        l1 = tf.layers.dense(pref, num_units, activation=tf.nn.relu)
        l2 = tf.layers.dense(l1, num_units, activation=tf.nn.relu)
        l3 = tf.layers.dense(l2, num_units, activation=tf.nn.relu)
        l4 = tf.layers.dense(l3, num_units, activation=tf.nn.relu)
       
        
        encode_input = tf.layers.batch_normalization(l4, axis=2, training=is_training, name='layer_norm', reuse=None)
...
def full_glimpse(ref, W_ref_g, W_q_g,v_g,   from_, to_, initializer=tf.contrib.layers.xavier_initializer()):
