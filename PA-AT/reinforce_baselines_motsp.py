...
import copy
from train_motsp import rollout, get_inner_model, get_angles
...
class RolloutBaseline(Baseline):
    ...
    def _update_model(self, model, epoch, dataset=None):
        ...
        self.bl_vals = rollout(self.model, prefa, 20, self.dataset, self.opts).cpu().numpy()
        self.mean = self.bl_vals.mean()
        self.epoch = epoch

    def wrap_dataset(self, dataset, pref,num_pref):
        ...
        return BaselineDataset(dataset, rollout(self.model, pref, num_pref, dataset, self.opts).view(-1, 1))

    def unwrap_batch(self, batch):
        return batch['data'], batch['baseline'].view(-1)  # Flatten result to undo wrapping as 2D

    def eval(self, pref, x, lamm, c):
        # Use volatile mode for efficient inference (single batch so we do not use rollout function)
        with torch.no_grad():
            ob1, ob2, ob3, cstr, _ = self.model(pref,x,lamm)
            #v, _ = self.model(x)

        # There is no loss
        return ob1, ob2, ob3, cstr

    def epoch_callback(self, model, pref, num_pref, epoch):
        """
        ...
        candidate_vals = rollout(model, pref, num_pref, self.dataset, self.opts).cpu().numpy()
